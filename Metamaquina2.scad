...
y_bushing_rod_height = 14;

YPlatform_height = Y_rod_height + y_bushing_rod_height;
pcb_height = YPlatform_height + 10;
BuildPlatform_height = pcb_height+2;

//machine_x_dim is the actual width of the whole machine
machine_x_dim = Z_rods_distance+2*(y_bushing_rod_height+thickness);
XPlatform_height = 45;
...
lm8uu_hole_offset = y_bushing_rod_height + 1;
XEnd_extra_width = 30;
XEnd_box_size = y_bushing_rod_height + z_rod_z_bar_distance + ZLink_rod_height;
...
//ArcPanel_height = 220; //TODO: make it depend on the machine height
module MachineArcPanel_long_face(){
  render(){
    difference(){
      union(){
      ...
      //hole for what? extruder wiring, maybe?
      //This is copied from the PrusaAir2 design
      translate([0, ArcPanel_height-27])
      rounded_square([ArcPanel_width-60,10], corners=[5,5,5,5], center=true);
...
%  import("Makerbot-MountPlate.stl");
}
...
module Z_couplings(){
  if (preview_ABS){
    color(ABS_color){
      translate([-machine_x_dim/2 + thickness + y_bushing_rod_height + z_rod_z_bar_distance, 0, BottomPanel_zoffset + motor_shaft_length])
      coupling_pair();
...
module XCarriage_lm8uu_holders(){
  if( preview_ABS ){
    color(ABS_color){
      translate([XCarPosition - XCarriage_lm8uu_distance/2, X_rods_distance/2, thickness + y_bushing_rod_height + X_rod_height])
      rotate([180,0,0])
      rotate([0,0,90])
      ybushing();
...
module belt_clamps(){
  if (preview_ABS){
    color(ABS_color){
      translate([XCarPosition - XCarriage_lm8uu_distance/2-10,  XPlatform_width/2 + XEnd_extra_width - belt_offset + belt_width/2, belt_clamp_height + 2*thickness + X_rod_height + y_bushing_rod_height])
      rotate([0,0,90])
      rotate([180,0,0])
      beltclamp();
...
module XEndMotor_linear_bearings(){
  if (preview_metal){
    color(metal_color){
	    translate([thickness + y_bushing_rod_height, 0, -12]){
      	cylinder(r=8, h=24);
...
module XEndMotor_ZLink(){
  translate([thickness + y_bushing_rod_height + ZLink_rod_height + z_rod_z_bar_distance, 0, thickness + (XPlatform_height-thickness)/2])
  rotate([0,0,-90])
  rotate([90,0,0])
  ZLink();
}
...
module ZRods(){
  if (preview_metal){
    color(metal_color){
      translate([-machine_x_dim/2 + thickness + y_bushing_rod_height, 0, BottomPanel_zoffset])
...
module WadeExtruder(){
  translate([XCarPosition, 0, 2*thickness + X_rod_height + y_bushing_rod_height])
  wade_block();
}

module XCarriage(){
  //lasercut parts:
  translate([XCarPosition, 0, thickness + X_rod_height + y_bushing_rod_height]){
...
module RodEndBottom_x2(){
  translate([0,0]) RodEndBottom_face(); //Z bottom left
  translate([0,30]) RodEndBottom_face(); //Z bottom right
}

module RodEndTop_x2(){
  translate([0,0]) RodEndTop_face(); //Z top left
  translate([0,30]) RodEndTop_face(); //Z top right
}

module export_individual_dxf_files(){
  export_curves(file = "/tmp/export/RightPanel.dxf")
  MachineRightPanel_face();

  export_curves(file = "/tmp/export/LeftPanel.dxf")
  MachineLeftPanel_face();

  export_curves(file = "/tmp/export/ArcPanel.dxf")
  MachineArcPanel_face();

  export_curves(file = "/tmp/export/BottomPanel.dxf")
  MachineBottomPanel_face();

  export_curves(file = "/tmp/export/TopPanel.dxf")
  MachineTopPanel_face();

  export_curves(file = "/tmp/export/XPlatformBottom.dxf")
  XPlatform_bottom_face();

  export_curves(file = "/tmp/export/XEndMotor_back.dxf")
  XEnd_back_face();

  export_curves(file = "/tmp/export/XEndMotor_front.dxf")
  XEnd_front_face();

  export_curves(file = "/tmp/export/XEndMotor_belt.dxf")
  XEndMotor_belt_face();

  export_curves(file = "/tmp/export/XEndMotor_plain.dxf")
  XEndMotor_plain_face();

  export_curves(file = "/tmp/export/XEndIdler_back.dxf")
  XEnd_back_face();

  export_curves(file = "/tmp/export/XEndIdler_front.dxf")
  XEnd_front_face();

  export_curves(file = "/tmp/export/XEndIdler_belt.dxf")
  XEndIdler_belt_face();

  export_curves(file = "/tmp/export/XEndIdler_plain.dxf")
  XEndIdler_plain_face();

  export_curves(file = "/tmp/export/XCarriage_bottom.dxf")
  XCarriage_bottom_face();
  
  export_curves(file = "/tmp/export/YPlatform_sheet.dxf")
  YPlatform_sheet_curves();

  export_curves(file = "/tmp/export/RodEndBottom_x2.dxf")
  RodEndBottom_x2();

  export_curves(file = "/tmp/export/RodEndTop_x2.dxf")
  RodEndTop_x2();

  export_curves(file = "/tmp/export/YMotorHolder.dxf")
  YMotorHolder_face();
}

//alphacortelaser Tel. (11) 3916-6452 
// 2,5 1,30
// cast / extrusado
//MDF
//PS - branco talvez
//PC um pouco mais duro
...
//TODO: figure out the usual plate dimentions for laser cutting service providers 

// alpha corte laser: 2500 1300 
  margin=20;
  //difference(){
    //translate([-margin,-margin]) square([2500+2*margin, 1300+2*margin]);
    //square([2500, 1300]);
  //}
...
//export_individual_dxf_files();

//export plates for laser cutting all parts:
//export_curves(file="/tmp/export/Metamaquina2-panel-1.dxf")
//lasercutter_panel1();

//export_curves(file="/tmp/export/Metamaquina2-panel-2.dxf")
//lasercutter_panel2();
